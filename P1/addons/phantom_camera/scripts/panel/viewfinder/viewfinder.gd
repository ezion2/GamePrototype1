...
func _set_viewfinder_state() -> void:
	...
	if is_instance_valid(_active_pcam):
		if _active_pcam.get_follow_mode() == _active_pcam.FollowMode.FRAMED or \
		_active_pcam.get_follow_mode() == _active_pcam.FollowMode.HYBRID:
...
func _on_dead_zone_changed() -> void:
	if not is_instance_valid(_active_pcam): return
	if not (_active_pcam.follow_mode == _active_pcam.FollowMode.FRAMED or \
			_active_pcam.follow_mode == _active_pcam.FollowMode.HYBRID): return
