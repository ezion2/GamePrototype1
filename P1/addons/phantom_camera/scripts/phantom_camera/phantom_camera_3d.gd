...
enum FollowMode {
	...
	HYBRID			= 7, ## Applies a [SpringArm3D] node to the target's position and allows for rotating around it. Applies a dead zone on the frame.
}
...
@export var follow_mode: FollowMode = FollowMode.NONE:
	set(value):
		...
		if follow_mode == FollowMode.FRAMED or follow_mode == FollowMode.HYBRID:
...
func _validate_property(property: Dictionary) -> void:
	...
	## Framed Follow
	###############
	if not (follow_mode == FollowMode.FRAMED or follow_mode == FollowMode.HYBRID):
		match property.name:
			"dead_zone_width", \
			"dead_zone_height", \
	...
	## Third Person Follow
	######################
	if not (follow_mode == FollowMode.THIRD_PERSON or follow_mode == FollowMode.HYBRID):
		match property.name:
			"spring_length", \
			"collision_mask", \
			"shape", \
...
func _ready():
	match follow_mode:
		FollowMode.THIRD_PERSON, FollowMode.HYBRID:
...
var _resume_vertical_tracking: bool = false
func _follow(delta: float) -> void:
	...
	match follow_mode:
		...
		# TODO: Add distance checks, fix weird spring arm clipping
		# TODO: fix collision
		# TODO: Custome deadzone, so we can set top and bottom deadzones independently
		FollowMode.HYBRID:
			if follow_target:
				# Check if we are not in the editor (i.e. running the game)
				if not Engine.is_editor_hint():
					# Ensure the follow target and spring arm are valid
					if is_instance_valid(follow_target) and is_instance_valid(_follow_spring_arm):
						# Assign the spring arm as the follow target for smooth camera movement
						## TODO: Lookahead is moving the spring arm
						## Should change follow offset instead, nevermind
						follow_target_node = _follow_spring_arm
						
						 # If the follow target is a CharacterBody3D (e.g. player character)
						if follow_target is CharacterBody3D or follow_target.player is CharacterBody3D:
							# Get the target position with an offsetet
							var target_position = _get_target_position_offset()
							
							# Calculate the center of the screen
							# Incorporate viewport position tracking from FRAMED mode
							# Calculate the target's position in viewport coordinates
							# Unproject the target position into the viewport (2D screen space)
							viewport_position = get_viewport().get_camera_3d().unproject_position(target_position)
							
							# Get the visible size of the viewport (screen space dimensions)
							#var visible_rect_size: Vector2 = get_viewport().get_viewport().size
							var visible_rect_size: Vector2 = get_viewport().get_visible_rect().size
							
							# Normalize the viewport position, converting it to a 0-1 range
							viewport_position = viewport_position / visible_rect_size
							
							# Store the current camera rotation (radians) for comparison later (optional, based on your future needs)
							_current_rotation = global_rotation
							
							# Check if the follow target (character) is on the ground
							var is_on_ground = follow_target.is_on_floor() if follow_target is CharacterBody3D else follow_target.player.is_on_floor()
							
							# Store the current Y-position of the follow target node (spring arm)
							var last_y : float = follow_target_node.global_position.y
							
							# If the target is on the ground
							# Use basic third person movement mode
							if is_on_ground:
								follow_position = target_position 
								
								# Disable vertical tracking
								_resume_vertical_tracking = false  # Reset vertical tracking flag when landed
							else:
								# If the target is in the air, follow only the X and Z, but keep the last Y position
								follow_position = Vector3(target_position.x, last_y, target_position.z)
								
								# Check if the character is outside the frame boundaries (for horizontal/vertical tracking)
								# Check if the target is outside the frame boundaries (could trigger tracking adjustments)
								var frame_out_bounds = _get_framed_side_offset()
								
								# If the character has moved outside the vertical deadzone, resume vertical tracking
								if frame_out_bounds.y != 0:
									_resume_vertical_tracking = true
								
								# If vertical tracking is enabled, update the camera's Y position
								if _resume_vertical_tracking:
									follow_position.y = target_position.y
						else:
							# Fallback: If the follow target is not a CharacterBody3D, use a simple third-person mode
							follow_position = _get_target_position_offset()
					else:
						# If the follow target or spring arm is not valid, use a default offset position
						follow_position = _get_position_offset_distance()
				else:
					# In the editor, use a simpler position offset for preview purposes
					follow_position = _get_position_offset_distance()
...
func _check_physics_body(target: Node3D) -> void:
	if target is PhysicsBody3D:
		...
		elif target is not CharacterBody3D and follow_mode == FollowMode.HYBRID:
			print_rich("Hybrid mode is for Characterbody3D only, reverting to standard Third Person mode.")
		return
